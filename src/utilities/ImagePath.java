 ...
 * @author OlilO
 */
public class ImagePath extends JPanel{
	...
	/**
	 * This method returns the VirtualPoint of this ImagePath at the specified x- and
	 * y-coordinate. If such a VirtualPoint doesn't exist, this method returns null.
	 * @param x : the x-coordinate of the VirtualPoint in this ImagePath
	 * @param y : the y-coordinate of the VirtualPoint in this ImagePath
	 * @return : The VirtualPoint at the given (x,y)-point in this ImagePath or null,
	 * 			if such a point doesn't exist.
	 */
	public VirtualPoint get(int x, int y) {
		if ((0 < x) && (x < width) && (0 < y) && (y < height) && content[x][y]) {
			for (int i = 0; i < vps.size(); i++) {
				if (((int)vps.get(i).getX() == x) && ((int)vps.get(i).getY() == y)) {
					return vps.get(i);
				}
			}
		}
	...
	public ImagePath merge(ImagePath path) {
		ImagePath ret = null;
		...
		for (int x = 0; x < ret.width; x++) {
			for (int y = 0; y < ret.height; y++) {
				ret.content[x][y] |= path.content[x][y];
				if (!ret.content[x][y] && path.content[x][y]) {
					ret.length++;
					ret.content[x][y] = true;
					ret.vps.add(
							path.get(x, y)
							//new VirtualPoint(x, y, Const.stoneWidth, Const.stoneHeight)
							);
				}
			}
		}
